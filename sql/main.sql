...
-- 2. SKILLS (Depends on Attributes)
CREATE TABLE Skills (
...
-- 3. ITEMS (Depends on Culture and Item Types)
CREATE TABLE Items (
...
-- 4. ITEM SUB-TYPES (1:1 with Items)
...
-- 6. FACTIONS & CLANS (Circular Dependency Handled)
...
-- 8. TROOPS & JUNCTION TABLES
CREATE TABLE Troops (
    troop_id INT AUTO_INCREMENT PRIMARY KEY,
    name VARCHAR(100) NOT NULL,
    tier INT NOT NULL,
    wage INT NOT NULL,
    is_mounted BOOLEAN NOT NULL,
    Culture_Type_ID INT,
    FOREIGN KEY (Culture_Type_ID) REFERENCES Culture_Types(Culture_Type_ID) ON DELETE SET NULL
) ENGINE=InnoDB;
...
CREATE TABLE Troop_Upgrade_Paths (
    ...
    FOREIGN KEY (base_troop_id) REFERENCES Troops(troop_id) ON DELETE CASCADE,
    FOREIGN KEY (upgraded_troop_id) REFERENCES Troops(troop_id) ON DELETE CASCADE
) ENGINE=InnoDB;